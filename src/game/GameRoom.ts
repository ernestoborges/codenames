...
export class GameRoom {
  ...
  private logSystemEvent(
    event: 'gameStart' | 'gameOver' | 'roomCreated' | 'gameReset' | 'teamsReset'
  ) {
  ...
  updatePlayerTeamAndRole(id: string, team: 0 | 1 | 2, role: string) {
    const player = this.players.find((p) => p.id === id)
    if (!player) return
    ...
    this.emitGameState(player.socket)
    console.log(
      `${player.username} atualizado: role[${player.role}] e team[${player.team}]`
    )
  }
  ...
  emitPlayers() {
    this.players.forEach((me) => {
      ...
      if (me.socket) this.io.to(me.socket).emit('roomPlayers', response)
    })
  }
  ...
  emitGameState(playerSocket?: string) {
    this.players.forEach((p) => {
      const gameStateToSend = {
        ...
        board:
          p.role === 'spymaster' || this.gameState.winner
            ? this.gameState.board
            : this.gameState.getOperativeCards(),
        spymasterTurn:
          p.role === 'spymaster' &&
          this.gameState.turn === p.team &&
      ...
      if (!playerSocket || playerSocket === p.socket)
        if (p.socket) this.io.to(p.socket).emit('gameState', gameStateToSend)
    })
  }
  ...
  startGame() {
    ...
    this.players.forEach((p) => {
      if (p.team === 1) {
        if (p.role === 'operative' && !minimumPlayers.o1)
          minimumPlayers.o1 = true
        if (p.role === 'spymaster' && !minimumPlayers.s1)
          minimumPlayers.s1 = true
      } else if (p.team === 2) {
        if (p.role === 'operative' && !minimumPlayers.o2)
          minimumPlayers.o2 = true
        if (p.role === 'spymaster' && !minimumPlayers.s2)
          minimumPlayers.s2 = true
      }
    })
